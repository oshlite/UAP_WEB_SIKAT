...
if ($_SERVER['REQUEST_METHOD']==='POST') {
  ...
  $area  = $_POST['area'];
  $prov  = $_POST['luar_provinsi'] ?? 'Tidak';

  // simpan
  $stmt = $pdo->prepare(
    "INSERT INTO tamu (nama, keperluan, area, luar_provinsi)
     VALUES (?,?,?,?)"
  );
  $stmt->execute([$nama,$keperluan,$area,$prov]);

  $success      = true;
  $guestName    = htmlspecialchars($nama);
  $areaRedirect = htmlspecialchars($area);
}
?>
<!DOCTYPE html>
<html lang="id">
...
<body class="bg-peach bg-opacity-20 min-h-screen flex flex-col">
  ...
  <main class="flex-1 w-full max-w-3xl mx-auto px-6 py-12">
    ...
    <header class="text-center mb-10">
      ...
      <p class="text-gray-600">
        Isi data diri Anda & petugas akan mengarahkan Anda.
      </p>
    </header>
    ...
    <section class="bg-white rounded-3xl shadow-lg p-8">
      <form method="post" class="space-y-6">
        ...
        <div>
          ...
          <div class="flex items-center space-x-6">
            <label class="flex items-center space-x-2">
              <input type="radio" name="luar_provinsi" value="Ya"
                     class="form-radio h-5 w-5 text-gold"/>
              <span class="text-gray-700">Ya</span>
            </label>
            <label class="flex items-center space-x-2">
              <input type="radio" name="luar_provinsi" value="Tidak" checked
                     class="form-radio h-5 w-5 text-gold"/>
        ...
        <!-- Submit -->
        <button type="submit"
          class="w-full bg-gold text-white py-3 rounded-xl font-medium
  ...
  <div id="redirectModal"
       class="fixed inset-0 bg-black bg-opacity-50 flex items-center
              justify-center z-50 opacity-0 pointer-events-none
              transition-opacity">
    <div class="bg-white rounded-2xl p-6 w-11/12 max-w-md text-center relative">
      ...
      <p class="text-gray-600 mb-4">
        Silakan menuju <span class="font-semibold text-gray-800">
          <?= $areaRedirect ?>
        </span>.<br/>
        Petugas akan membantu Anda.
      </p>
      <button id="modalOk"
        class="bg-gold text-white px-6 py-2 rounded-xl
               hover:bg-gold-dark transition">
        Oke
      </button>
    </div>
